...
class OptimizedBacktester:
    ...
    def calculate_metrics(self):
        ...
        for quarter in sorted(df['quarter'].unique()):
            ...
            q_capital = q_df['position_value'].sum()
            q_pnl = q_df['total_pnl'].sum()
            q_return_pct = (q_pnl / q_capital) * 100 if q_capital > 0 else 0
            ...
            by_quarter.append({
                ...
                'Capital Deployed $': q_capital,
                'Total P&L $': q_pnl,
                'Quarter Return %': q_return_pct,
    ...
    def print_report(self, overall, by_tf, by_type, by_tf_type, by_quarter):
        ...
        for row in by_quarter:
            print(f"  {row['Quarter']}: {row['Total Trades']} trades | W: {row['Winners']} | WR: {row['Win Rate %']:.1f}% | "
                  f"Capital: ${row['Capital Deployed $']:,.2f} | P&L: ${row['Total P&L $']:,.2f} | Return: {row['Quarter Return %']:.2f}%")
        print()
